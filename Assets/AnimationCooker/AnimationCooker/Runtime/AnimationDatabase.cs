...
//   (don't use funky special chars, don't start with numbers, don't use unity keywords, don't use spaces)
//
...
// It contains redudant information, but it's there for convenience
// and since it's store in only one place in the database, it's not really wasting much space.
// It must be a non-nullable type (ie can't contain classes, strings, etc) so that it can be stored
// in an UnsafeList.
